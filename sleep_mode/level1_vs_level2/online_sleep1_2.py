...
def test_with_sleep_mode(model1, model2, prompt, level: int = 1):
    ...
    if first_model and first_model_url:
        ...
        sleep_model(first_model_url, level=level)
        ...
        if second_model and second_model_url:
            ...
            sleep_model(second_model_url, level=level)

            event_prefix_a = "A Model" if level == 1 else "A"
            event_prefix_b = "B Model" if level == 1 else "B"

            # Round 1
            print("\n--- Round 1 ---")
            if level == 1:
                wake_model_timed(first_model_url, f"{event_prefix_a} Wake Up (1)")
            else:
                wake_and_reload_then_warmup_timed(first_model_url, model1, f"{event_prefix_a} Reload Weights(1)", do_warmup=True)
            prompt_model_timed(first_model_url, model1, prompt1, f'{event_prefix_a} Model Prompt (1)')
            sleep_model_timed(first_model_url, f"{event_prefix_a} Model Sleep (1)", level=level)
            
            if level == 1:
                wake_model_timed(second_model_url, f"{event_prefix_b} Wake Up (1)")
            else:
                wake_and_reload_then_warmup_timed(second_model_url, model2, f"{event_prefix_b} Reload Weights(1)", do_warmup=True)
            prompt_model_timed(second_model_url, model2, prompt1, f'{event_prefix_b} Model Prompt (1)')
            sleep_model_timed(second_model_url, f"{event_prefix_b} Model Sleep (1)", level=level)
            
            # Round 2
            print("\n--- Round 2 ---")
            if level == 1:
                wake_model_timed(first_model_url, f'{event_prefix_a} Wake Up (2)')
            else:
                wake_and_reload_then_warmup_timed(first_model_url, model1, f'{event_prefix_a} Reload Weights(2)', do_warmup=True)
            prompt_model_timed(first_model_url, model1, prompt2, f'{event_prefix_a} Model Prompt (2)')
            sleep_model_timed(first_model_url, f"{event_prefix_a} Model Sleep (2)", level=level)
            
            if level == 1:
                wake_model_timed(second_model_url, f'{event_prefix_b} Wake Up (2)')
            else:
                wake_and_reload_then_warmup_timed(second_model_url, model2, f'{event_prefix_b} Reload Weights(2)', do_warmup=True)
            prompt_model_timed(second_model_url, model2, prompt2, f'{event_prefix_b} Model Prompt (2)')
            sleep_model_timed(second_model_url, f"{event_prefix_b} Model Sleep (2)", level=level)
            
            # Round 3
            print("\n--- Round 3 ---")
            if level == 1:
                wake_model_timed(first_model_url, f'{event_prefix_a} Wake Up (3)')
            else:
                wake_and_reload_then_warmup_timed(first_model_url, model1, f'{event_prefix_a} Reload Weights(3)', do_warmup=True)
            prompt_model_timed(first_model_url, model1, prompt3, f'{event_prefix_a} Model Prompt (3)')
            sleep_model_timed(first_model_url, f"{event_prefix_a} Model Sleep (3)", level=level)
            
            if level == 1:
                wake_model_timed(second_model_url, f'{event_prefix_b} Wake Up (3)')
            else:
                wake_and_reload_then_warmup_timed(second_model_url, model2, f'{event_prefix_b} Reload Weights(3)', do_warmup=True)
            prompt_model_timed(second_model_url, model2, prompt3, f'{event_prefix_b} Model Prompt (3)')
...
# Run in sleep mode level1, command execution: python online_sleep1_2.py sleep1
# Run in sleep mode level2, command execution: python online_sleep1_2.py sleep2
def main():
    ...
    if test_mode == 'sleep1':
        level_to_run = 1
    elif test_mode == 'sleep2':
        level_to_run = 2
    else:
        print("Error: Please specify a mode.", file=sys.stderr)
        print("Usage: python online_sleep1_2.py [sleep1 | sleep2]", file=sys.stderr)
        sys.exit(1)
    
    try:

        print(f"=== CONFIG: Running in SLEEP MODE (Level {level_to_run}) ===")
        sleep_start = time.time()
        test_with_sleep_mode(model1, model2, prompt, level=level_to_run)
